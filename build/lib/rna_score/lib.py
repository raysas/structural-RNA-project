...
def download_rna_structures(
    ...
    # -- both development (src/) and installed (site-packages/) locations tral
    script_candidates = [
        Path(__file__).parent.parent / "access_rna_structures.py",  # Installed
        Path(__file__).resolve().parents[2] / "src" / "access_rna_structures.py",  # Development
    ]
    ...
    if script_path is None:
        raise FileNotFoundError(
            f"access_rna_structures.py not found. Searched:\n" +
            "\n".join(f"  - {c}" for c in script_candidates)
        )
    
    cmd = [
        sys.executable,
        str(script_path),
        "-n", str(max_structures),
        "-f"] + formats + [
        "-o", str(output_dir),
        "-w", str(workers)
    ]
    ...
    id_file = output_dir / ("pdb_ids.txt" if list_only else "downloaded_ids.txt")
    if id_file.exists():
        pdb_ids = id_file.read_text().strip().split("\n")
    else:
        pdb_ids = []
